...
if __name__ == "__main__":
    ...
    interval = pd.Timedelta(f"{args.multiplier} {args.timespan}")
    ...
    print(f"Downloading data for {len(args.symbols)} symbols...")
    for i, symbol in enumerate(args.symbols):
        try:
            print(f"Fetching data for {symbol} ({i+1}/{len(args.symbols)})...")
            ...
            logging.info(
                f"Saving data for {symbol} to {args.destination_folder}/{symbol}.csv with {len(df)} rows"
            )
            df.to_csv(f"{args.destination_folder}/{symbol}.csv", index=False)
            ...
            #     df, f"{args.destination_folder}/{symbol}_weekly_distribution.png"
            # )
            # plot_overall_distribution(
            #     df, f"{args.destination_folder}/{symbol}_overall_distribution.png"
            # )
